...
public class TermsService {
	...
	public Long createTerms(AdminCreateTermsRequest request) {
		...
		AdminTermsDto createAdminTermsDto = fromAdminCreateTermsRequestAndVersion(request, ++maxVersion);

		try {
			return adminTermsRepository.save(createAdminTermsDto).id();
	...
	public Long updateTerms(@Valid AdminUpdateTermsRequest request) {
		AdminTermsDto findAdminTermsDto = adminTermsRepository.findById(request.id())
			.orElseThrow(() -> new IllegalArgumentException("약관이 존재하지 않습니다."));

		AdminTermsDto updateAdminTermsDto = fromAdminUpdateTermsRequestAndVersion(request, 1);

		return OptimisticLockingFailureRetryUtils.executeWithRetry(MAX_TERMS_SAVE_OPTIMISTIC_LOCK_RETRY_COUNT,
			() -> adminTermsRepository.save(updateAdminTermsDto).id());
	}
}
